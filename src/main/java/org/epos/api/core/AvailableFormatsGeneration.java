...
public class AvailableFormatsGeneration {
	...
	public static List<AvailableFormat> generate(Distribution distribution) {
		...
		if (distribution.getSupportedOperation() != null) {
			...
			// AbstractAPI.retrieveAPI(EntityNames.OPERATION.name()).retrieveBunch(distribution.getSupportedOperation().stream().map(LinkedEntity::getInstanceId).collect(Collectors.toList()));
			List<Operation> operationList = DatabaseConnections.getInstance().getOperationList().stream()
					.filter(item -> distribution.getSupportedOperation().stream().map(LinkedEntity::getInstanceId)
							.collect(Collectors.toList()).contains(item.getInstanceId()))
					.collect(Collectors.toList());
			for (Operation operation : operationList) {
				boolean isOgcFormat = false;
				// Operation operation = (Operation)
				// AbstractAPI.retrieveAPI(EntityNames.OPERATION.name()).retrieve(supportedOperationLE.getInstanceId());

				if (operation != null && operation.getMapping() != null) {
					// List<Mapping> mappings = operation.getMapping().stream()
					// .map(mappingLE -> (Mapping)
					// AbstractAPI.retrieveAPI(EntityNames.MAPPING.name()).retrieve(mappingLE.getInstanceId()))
					// .collect(Collectors.toList());
					// List<Mapping> mappings =
					// AbstractAPI.retrieveAPI(EntityNames.MAPPING.name()).retrieveBunch(operation.getMapping().stream().map(LinkedEntity::getInstanceId).collect(Collectors.toList()));
					List<Mapping> mappings = DatabaseConnections.getInstance().getMappingList().stream()
							.filter(item -> operation.getMapping()
									.stream()
		...
		return mappings.stream()
				.anyMatch(e -> e.getVariable().equalsIgnoreCase("service")
						&& (map.getParamValue().contains(service)
								|| e.getDefaultValue().toLowerCase().contains(service)));
	}
