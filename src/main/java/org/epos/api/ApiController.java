...
import org.epos.configuration.RedisConfiguration;
...
abstract class ApiController<T> {
	...
	protected ResponseEntity<T> standardRequest(ServiceType service, Map<String, Object> requestParams, CacheDataRepository cacheDataRepository) {
		...
		try {
			...
			if (optionalCacheData.isPresent()) {
				return (ResponseEntity<T>) ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body(optionalCacheData.get().getValue());
			}
		}catch(Exception e) {
				...
				if(Utils.gson.fromJson(response.getPayloadAsPlainText().get(), JsonElement.class).isJsonObject()) {
					if(!Utils.gson.fromJson(response.getPayloadAsPlainText().get(), JsonElement.class).getAsJsonObject().entrySet().isEmpty()) {
						...
						if(hasContent(payload) ) {
							try {
								CacheData cacheData = new CacheData(cacheId, payload.get());
						...
						return hasContent(payload)? (ResponseEntity<T>) ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body(payload.get()) : ResponseEntity.status(HttpStatus.NO_CONTENT).build();
					}
				}
